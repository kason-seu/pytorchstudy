...
print(x)
y = x.mm(torch.tensor([[-1.0,1.0],[3.0,2.56]]))
print(y)
class Data(Dataset):
    def __init__(self):
...
opt = optim.SGD(params=lr.parameters(), lr = 0.01)
LOSS = []
...
print('result', yhat_result.detach()[:,0:1])

plt.plot(data.y.numpy()[:,0:1], 'b*')
plt.plot(yhat_result.detach().numpy()[:,0:1])
plt.show()
plt.plot(data.y.numpy()[:,1:2], 'b*')
plt.plot(yhat_result.detach().numpy()[:,1:2])
plt.show()